...
describe('auth', () => {
  ...
  test('should accept HTTP Basic auth with username/password', () => {
    axios('/foo', { auth: { username: 'tom', password: 'cat' } })

    return getAjaxRequest().then(req => {
      expect(req.requestHeaders['Authorization']).toBe('Basic dG9tOmNhdA==')
    })
  })

  test('should fail to encode HTTP Basic auth credentials with non-Latin1 characters', () => {
    return axios('/foo', { auth: { username: 'Aladßç£☃din', password: 'open sesame' } })
      .then(() => {
        throw new Error(
