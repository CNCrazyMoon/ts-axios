...
describe('mergeConfig', () => {
  ...
  test('should accept undefined form second argument', () => {
  ...
  test('should merge auth, headers with defaults', () => {
    expect(
      mergeConfig({ auth: undefined }, { auth: { username: 'foo', password: 'bar' } })
    ).toEqual({ auth: { username: 'foo', password: 'bar' } })

    expect(
      mergeConfig(
        { auth: { username: 'foo', password: 'bar' } },
        { auth: { username: 'tom', password: 'cat' } }
      )
    ).toEqual({ auth: { username: 'tom', password: 'cat' } })
  })
